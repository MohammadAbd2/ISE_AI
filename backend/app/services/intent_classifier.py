...
class IntentClassifier:
    ...
    FILESYSTEM_TERMS = (
        ...
        "search in files", "find in files",
        "replace in files", "search and replace", "find and replace",
        "file tree", "project tree", "directory tree",
        # Write operations (file manipulation)
        "delete file", "delete the file", "remove file", "remove the file",
        "rename file", "rename the file", "copy file", "copy the file",
        "write to file", "write content to", "save to file",
        "update file", "update the file", "edit file", "edit the file",
    )
    CODING_TERMS = (
        ...
        "build component", "render component", "frontend component", "ui component",
        "build this in the frontend", "put this in the frontend", "start coding",
    )
    CHAT_TERMS = (
    ...
    def _classify_auto(self, lower: str, force_chat: bool) -> TaskIntent:
        if not force_chat and self._looks_like_visual_implementation(lower):
            return TaskIntent(kind="coding", confidence=0.9, use_agent=True)
    ...
    def _looks_like_visual_implementation(self, lower: str) -> bool:
        visual = any(term in lower for term in self.VISUAL_TERMS)
        implementation = any(
            term in lower
            for term in (
                "component",
        ...
        action = any(
            term in lower
            for term in ("create", "build", "implement", "put", "make", "develop", "render")
        )
        return visual and implementation and action
