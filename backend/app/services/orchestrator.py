...
class ResearchAgent:
    ...
    async def run(self, session_id: str | None, user_message: str) -> OrchestratorResult:
        ...
        if not self.search_service.should_search(user_message):
            ...
            return OrchestratorResult()
        try:
            # Use a strict timeout for the search operation to prevent frontend disconnects
            log = await asyncio.wait_for(
                self.search_service.search(session_id, query=user_message),
                timeout=35.0
            )
        except asyncio.TimeoutError:
            log = self.search_service.failed_log(user_message, "Search timed out. Try a more specific query.")
...
class CapabilityAgent:
    ...
    async def run(self, session_id: str | None, user_message: str) -> OrchestratorResult:
        ...
        if self._wants_session_analytics(lower):
            payload = await self._execute_registry_tool(
                "session_analytics",
                session_id=session_id,
            )
            if payload.get("status") == "failed":
                return OrchestratorResult(
                    direct_reply=f"Session analytics is unavailable right now: {payload['error']}",
                    used_agents=[self.name],
                )
        ...
        if self._wants_research_memory(lower):
            ...
            if result.get("status") == "failed":
                return OrchestratorResult(
                    direct_reply=f"Could not reopen research memory: {result['error']}",
                    used_agents=[self.name],
                )
            return OrchestratorResult(
        ...
        if self._wants_session_history(lower):
            ...
            if payload.get("status") != "success":
                return OrchestratorResult(
                    direct_reply=f"Session history is unavailable right now: {payload.get('error', 'unknown error')}",
                    used_agents=[self.name],
                )
            return OrchestratorResult(
                direct_reply=(
    ...
    async def _execute_registry_tool(self, tool_name: str, **kwargs) -> dict:
        try:
            result = await self.registry.execute_tool_async(tool_name, **kwargs)
        except Exception as exc:
...
class CodingAgent:
    ...
    def should_code(self, user_message: str) -> bool:
        ...
        self_improvement_dev_triggers = [
            ...
            "monitor and adjust",
            "improve your output style",
            "better output style",
            "response formatter",
            "output formatter",
        ]
        if any(trigger in lower for trigger in self_improvement_dev_triggers):
            return True
    ...
    async def run(self, session_id: str | None, user_message: str, attachments: list[ChatAttachment] | None = None, progress_callback=None) -> OrchestratorResult:
        ...
        try:
            self.coding_agent.set_progress_callback(None)
            self.planning_agent.set_progress_callback(None)
            # Check if task has multiple steps (needs planning)
            has_multiple_steps = self._has_multiple_steps(user_message)

            if has_multiple_steps:
                if progress_callback:
                    async def planning_progress(plan):
                        await progress_callback(self._build_planning_progress_block(plan))
                    self.planning_agent.set_progress_callback(planning_progress)
                ...
                if plan.status.value == "completed":
                    return OrchestratorResult(
                        direct_reply=(
                            f"✅ **Task complete**\n\n"
                            f"{plan.completed_steps}/{plan.total_steps} steps finished."
                        ),
                        used_agents=[self.name],
                        render_blocks=self._build_render_blocks(
                            ...
                            plan.status.value,
                            progress_message=f"{plan.completed_steps}/{plan.total_steps} steps finished.",
                            plan_steps=[
                                {
                    ...
                    return OrchestratorResult(
                        direct_reply=(
                            f"⚠️ **Task status**\n\n"
                            f"{plan.completed_steps}/{plan.total_steps} steps completed. Review the execution blocks below."
                        ),
                        used_agents=[self.name],
                        render_blocks=self._build_render_blocks(
                            plan.task,
                            [],
                            plan.status.value,
                            progress_message=f"{plan.completed_steps}/{plan.total_steps} steps completed.",
                            plan_steps=[
                                {
                                    "step_number": step.step_number,
                                    "description": step.description,
                ...
                if progress_callback:
                    async def coding_progress(progress, message):
                        await progress_callback(self._build_coding_progress_block(progress, message))
                    self.coding_agent.set_progress_callback(coding_progress)
                ...
                project_context = await self._load_project_context(session_id)
                # coding_agent.execute_task currently accepts only the task description; provide project_context via agent methods if needed
                ...
                if progress.overall_status == "completed":
                    return OrchestratorResult(
                        direct_reply=(
                            f"✅ **Development task complete**\n\n"
                            f"{len(progress.files_modified)} file(s) changed. Verification passed."
                        ),
                        used_agents=[self.name],
                        render_blocks=self._build_render_blocks(
                    ...
                    return OrchestratorResult(
                        direct_reply=(
                            f"⚠️ **Task status**\n\n"
                            f"{progress.message} Review the execution and verification blocks below."
                        ),
                        used_agents=[self.name],
                        render_blocks=self._build_render_blocks(
                            progress.task_description,
        ...
        blocks = [
            {
                ...
                "payload": {
                    ...
                    "task": task,
                    "status": status,
                    "message": progress_message or f"Task status: {status}.",
                    "file_count": len(files),
                    "verification_count": len(verification_steps or []),
                    "execution_steps": (plan_steps or [])[:8],
                    "verification_steps": (verification_steps or [])[:6],
                },
            },
            {
                "type": "report",
                "payload": {
                    ...
                    "summary": progress_message or f"Task status: {status}.",
    ...
    def _build_coding_progress_block(self, progress, message: str) -> dict:
        steps = []
        for index, action in enumerate(progress.actions[-8:]):
            steps.append(
                {
                    "step": action.action_type,
                    "message": action.description,
                    "details": action.target,
                    "timestamp": action.completed_at or action.started_at or "",
                    "is_complete": action.status.value == "completed",
                    "is_error": action.status.value == "failed",
                    "status": action.status.value,
                    "output": action.output[:320] if action.output else "",
                    "error": action.error[:320] if action.error else "",
                    "diff": action.diff[:320] if action.diff else "",
                }
            )
        return {
            ...
            "payload": {
                ...
                "is_complete": progress.overall_status == "completed",
                "is_error": progress.overall_status == "failed",
            },
        }

    def _build_planning_progress_block(self, plan) -> dict:
        steps = []
            ...
            steps.append(
                {
                    ...
                    "details": step.target,
                    "timestamp": step.completed_at or step.started_at or "",
                    "is_complete": step.status.value == "completed",
                    "is_error": step.status.value == "failed",
                    "status": step.status.value,
                    "output": step.output[:320] if step.output else "",
                    "error": step.error[:320] if step.error else "",
                    "diff": "",
                }
            )
        latest_message = (
            f"Planning execution: {plan.completed_steps}/{plan.total_steps} steps completed"
            if plan.total_steps
            else "Planning execution in progress"
        )
        return {
            "type": "coding_progress",
            "payload": {
                ...
                "is_complete": plan.status.value == "completed",
                "is_error": plan.status.value == "failed",
            },
        }
...
class ImageGenerationAgent:
    ...
    def should_generate(self, user_message: str) -> bool:
        ...
        non_visual_self_improvement_indicators = [
            "develop yourself",
            "develop your self",
            "improve yourself",
            "improve your self",
        ...
        if any(indicator in lower for indicator in non_visual_self_improvement_indicators):
            return False
...
class MultiAgentOrchestrator:
    ...
    async def run(
        ...
        utility = await self._run_agent_safe(self.utility_agent.name, self.utility_agent.run(user_message))
        ...
        documents = await self._run_agent_safe(
            self.document_agent.name,
            self.document_agent.run(session_id, attachments, user_message),
        )
        aggregate.tool_context.extend(documents.tool_context)
        aggregate.used_agents.extend(documents.used_agents)

        urls = await self._run_agent_safe(self.url_agent.name, self.url_agent.run(session_id, user_message))
        aggregate.tool_context.extend(urls.tool_context)
        aggregate.used_agents.extend(urls.used_agents)

        images = await self._run_agent_safe(
            self.image_intel_agent.name,
            self.image_intel_agent.run(session_id, user_message, attachments),
        )
        ...
        capability = await self._run_agent_safe(
            self.capability_agent.name,
            self.capability_agent.run(session_id, user_message),
        )
        ...
        coding = await self._run_agent_safe(
            self.coding_agent.name,
            self.coding_agent.run(session_id, user_message, progress_callback=progress_callback),
        )
        ...
        gen_image = await self._run_agent_safe(
            self.image_generation_agent.name,
            self.image_generation_agent.run(session_id, user_message),
        )
        ...
        video = await self._run_agent_safe(
            self.video_generation_agent.name,
            self.video_generation_agent.run(session_id, user_message),
        )
        aggregate.tool_context.extend(video.tool_context)
        aggregate.used_agents.extend(video.used_agents)

        research = await self._run_agent_safe(
            self.research_agent.name,
            self.research_agent.run(session_id, user_message),
        )
        ...
        execution = await self._run_agent_safe(
            self.execution_agent.name,
            self.execution_agent.run(session_id, user_message),
        )
    ...
    async def _run_agent_safe(
        ...
        try:
            ...
            return OrchestratorResult(
                tool_context=[f"{agent_name} failed and was skipped: {exc}"],
                used_agents=[agent_name],
            )
