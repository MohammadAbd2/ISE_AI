...
class SearchService:
    ...
    _ENRICH_TOP_N = 2 # Reduced from 3 for better performance
    _DEEP_PAGE_CHARS = 2400 # Reduced from 2800
    _SHALLOW_PAGE_CHARS = 500 # Reduced from 560
    _SNIPPET_FALLBACK_CHARS = 450 # Reduced from 520
    _MAX_QUERY_VARIANTS = 3
    _FRESH_QUERY_CACHE_TTL_HOURS = 6
    _DEFAULT_CACHE_TTL_HOURS = 72
    _OFFICIAL_DOMAIN_HINTS = [
        (r"\bvite\b", ["vite.dev", "vitejs.dev"]),
        (r"\breact\b", ["react.dev"]),
        (r"\bnext(?:\.js|js)?\b", ["nextjs.org"]),
        (r"\bfastapi\b", ["fastapi.tiangolo.com"]),
        (r"\btypescript\b", ["typescriptlang.org"]),
        (r"\bnode(?:\.js|js)?\b", ["nodejs.org"]),
        (r"\bpython\b", ["docs.python.org", "python.org"]),
        (r"\btailwind(?:css)?\b", ["tailwindcss.com"]),
        (r"\bopenai\b", ["platform.openai.com", "openai.com"]),
        (r"\bpostgres(?:ql)?\b", ["postgresql.org"]),
        (r"\bdocker\b", ["docs.docker.com", "docker.com"]),
        (r"\bkubernetes\b", ["kubernetes.io"]),
    ]
    _TECHNICAL_PATH_HINTS = (
        "docs",
        "documentation",
        "reference",
        "api",
    ...
    def should_search(self, user_message: str) -> bool:
        ...
        if self._is_small_talk(lower):
            return False
        if any(trigger in lower for trigger in explicit_triggers):
            return True
        if any(trigger in lower for trigger in freshness_triggers) and any(
            modifier in lower for modifier in freshness_modifiers
        ):
            return True
    ...
    def _is_small_talk(self, lower: str) -> bool:
        return bool(
            re.search(
                r"\b("
                r"hi|hello|hey|good morning|good afternoon|good evening|"
                r"how are you|how are you doing|how's it going|how is it going|"
                r"how's your day|how is your day|what's up|whats up|"
                r"nice to meet you|thank you|thanks"
                r")\b",
                lower,
            )
        )
    ...
    async def search(self, session_id: str, query: str, limit: int = 5) -> WebSearchLog:
        ...
        if cached and self._should_use_cached_research(query, cached.created_at):
    ...
    def _should_use_cached_research(self, query: str, created_at: str) -> bool:
        try:
            cached_at = datetime.fromisoformat(created_at)
        except Exception:
            return not self._should_prioritize_freshness(query)
        ...
        ttl_hours = (
            self._FRESH_QUERY_CACHE_TTL_HOURS
            if self._should_prioritize_freshness(query)
            else self._DEFAULT_CACHE_TTL_HOURS
        )
    ...
    def build_render_blocks(self, log: WebSearchLog) -> list[dict]:
        ...
        render_blocks.append({
            ...
            "payload": {
                ...
                "confidence": confidence,
                "conflict": conflict,
                "comparison_summary": self._build_source_comparison(log.query, log.sources),
                "sources": [
                    {
                        "title": source.title,
                        "url": source.url,
                        ...
                        "reason": self._explain_source_fit(log.query, source),
    ...
    def _build_summary(self, prepared_query: str, sources: list[SearchSource]) -> str:
        ...
        fresh_summary = self._build_fresh_summary(prepared_query, sources)
        if fresh_summary:
    ...
    def _build_fresh_summary(self, prepared_query: str, sources: list[SearchSource]) -> str:
        if not self._should_prioritize_freshness(prepared_query):
            return ""
        ...
        for source in sources[:3]:
            domain = source.domain or self._extract_domain(source.url)
            freshness = self._extract_source_date_text(source)
            label = source.title or domain or source.url
            if freshness:
                lines.append(f"{label} ({domain}) [{freshness}]")
            else:
                lines.append(f"{label} ({domain})")

        freshness_overview = self._estimate_freshness(sources)
        if not lines:
            return ""

        if freshness_overview:
            return f"Freshest-looking sources: {'; '.join(lines)}. {freshness_overview}"
        return f"Freshest-looking sources: {'; '.join(lines)}."
    ...
    def _detect_numeric_conflict(self, sources: list[SearchSource]) -> str:
        ...
        if low <= 0:
            return ""
        spread = (high - low) / low
        if spread >= 0.25: # Increased from 0.1 to reduce noise
            return f"Note: sources show varying values ({low:,.0f} to {high:,.0f})"
        return ""
    ...
    def _authority_score(self, domain: str) -> float:
        ...
        if domain.endswith(".gov") or domain.endswith(".edu"):
            return 2.0
        if any(
            domain == hinted or domain.endswith(f".{hinted}")
            for _, hinted_domains in self._OFFICIAL_DOMAIN_HINTS
            for hinted in hinted_domains
        ):
            return 1.9
    ...
    def _build_query_variants(self, original_query: str, prepared_query: str) -> list[str]:
        ...
        inferred_domains = self._infer_official_domains(original_query)
        ...
        all_domains = list(dict.fromkeys([*preferred_domains, *inferred_domains]))

        if prefer_freshness and all_domains:
            for domain in all_domains[:1]:
                if f"site:{domain}" not in prepared_query:
                    add_variant(f"{prepared_query} latest {current_year} site:{domain}")

        if self._is_technical_docs_query(original_query) and all_domains:
            for domain in all_domains[:2]:
                if f"site:{domain}" not in prepared_query:
                    add_variant(f"{prepared_query} site:{domain}")

        if prefer_freshness:
            add_variant(f"{prepared_query} latest {current_year}")
            add_variant(f"{prepared_query} current {current_year}")

        return variants[: self._MAX_QUERY_VARIANTS]

    def _is_technical_docs_query(self, text: str) -> bool:
        lower = text.lower()
        markers = (
            "docs",
            "documentation",
    ...
    def _infer_official_domains(self, text: str) -> list[str]:
        lower = text.lower()
        if not self._is_technical_docs_query(lower):
            return []
        domains: list[str] = []
        for pattern, hinted_domains in self._OFFICIAL_DOMAIN_HINTS:
    ...
    def _score_result(
        ...
        source = SearchSource(title=title, url=url, snippet=snippet, domain=domain)
        score = self._technical_relevance_score(prepared_query, source)
        inferred_domains = self._infer_official_domains(prepared_query)
        if inferred_domains and any(domain.endswith(inferred) for inferred in inferred_domains):
            score += 4.5
        ...
        if any(marker in domain for marker in ("docs.", "developer.", "support.")):
            score += 0.75
        score += self._freshness_score(source, prefer_freshness)
        return score

    def _query_tokens(self, prepared_query: str) -> list[str]:
        ignored = {
            "latest",
            "current",
            "recent",
        ...
        return [
            token
            for token in re.findall(r"[a-z0-9]{3,}", prepared_query.lower())
            if token not in ignored
        ]

    def _technical_relevance_score(self, prepared_query: str, source: SearchSource) -> float:
        haystack = " ".join(part for part in [source.title, source.snippet, source.page_excerpt] if part).lower()
        path = urlparse(source.url).path.lower()
        tokens = self._query_tokens(prepared_query)
        overlap = sum(1 for token in tokens if token in haystack or token in path)
        score = float(overlap)

        if self._is_technical_docs_query(prepared_query):
            if any(hint in path or hint in haystack for hint in self._TECHNICAL_PATH_HINTS):
                score += 1.4
            if any(hint in path or hint in haystack for hint in self._LOW_SIGNAL_PATH_HINTS):
                score -= 0.6
            if "github.com" in (source.domain or "") and "/releases" in path:
                score += 0.9
            if any(marker in haystack for marker in ("official", "documentation", "reference")):
                score += 0.5

        return score

    def _freshness_score(self, source: SearchSource, prefer_freshness: bool) -> float:
        if not prefer_freshness:
            return 0.0
        haystack = " ".join(part for part in [source.title, source.snippet, source.page_excerpt] if part).lower()
        score = 1.0 if any(marker in haystack for marker in ("today", "latest", "updated", "current")) else 0.0
        parsed = self._extract_source_datetime(source)
        if parsed is None:
            return score
    ...
    def _build_source_comparison(self, query: str, sources: list[SearchSource]) -> str:
        if not sources:
            return ""

        strongest = max(
            sources[:6],
            key=lambda source: self._authority_score(source.domain or self._extract_domain(source.url)),
        )
        freshest = max(
            sources[:6],
            key=lambda source: self._extract_source_datetime(source) or datetime(1970, 1, 1, tzinfo=UTC),
        )
        relevance = max(
            sources[:6],
            key=lambda source: self._technical_relevance_score(query, source),
        )

        strongest_domain = strongest.domain or self._extract_domain(strongest.url) or strongest.url
        freshest_label = freshest.title or freshest.domain or freshest.url
        relevance_label = relevance.title or relevance.domain or relevance.url
        pieces = [f"Authority lead: {strongest_domain}", f"Best topical match: {relevance_label}"]
        freshest_date = self._extract_source_date_text(freshest)
        if freshest_date:
            pieces.append(f"Freshest date signal: {freshest_label} [{freshest_date}]")
        return ". ".join(pieces) + "."

    def _explain_source_fit(self, query: str, source: SearchSource) -> str:
        reasons: list[str] = []
        domain = source.domain or self._extract_domain(source.url)
        authority = self._authority_label(domain)
        if authority == "official":
            reasons.append("official source")
        elif authority == "documentation":
            reasons.append("documentation source")

        freshness = self._extract_source_date_text(source)
        if freshness:
            reasons.append(f"dated {freshness}")

        if self._technical_relevance_score(query, source) >= 2.5:
            reasons.append("strong keyword/path match")
