...
class IntelligentCodingAgent:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        # Workspace staging support (lazy)
        try:
    ...
    def enable_workspace(self, enable: bool = True):
        ...
        self.use_workspace = bool(enable)
        if enable and getattr(self, 'workspace', None) is not None:
            # workspace object provides base path via .base
            self._workspace_base = getattr(self.workspace, 'base', None)
        else:
            self._workspace_base = None
    ...
    def _determine_intent(self, task: str) -> str:
        ...
        analytics_markers = ["chart", "graph", "analytics", "analysis", "flight", "profit", "salary", "revenue"]
        if re.search(r"\bmap\b", task_lower) and "road map" not in task_lower and "roadmap" not in task_lower:
            analytics_markers.append("map")
        if any(kw in task_lower for kw in ["dashboard", "workspace", "component", "frontend", "chat"]) and any(
            kw in task_lower for kw in analytics_markers
        ):
    ...
    def _extract_component_name(self, task: str) -> str:
        """Extract a sensible component/file name from task text."""
        task_lower = task.lower()
        ...
        patterns = [
            r'(?:called|named|call it)\s+["\']?([A-Za-z0-9_\-]+(?:\.(?:js|jsx|ts|tsx|py|css|html|txt))?)["\']?',
            r'["\']([A-Za-z0-9_\-]+\.(?:js|jsx|ts|tsx|py|css|html|txt))["\']',
        ]
        ...
        message_content = self._extract_message_content(task)
        if message_content:
            normalized = self._normalize_identifier(message_content)
            if normalized and normalized != "Generated":
                return normalized

        words = re.findall(r"[A-Za-z0-9_]+", task)
        ignored = {
            "create", "make", "build", "file", "files", "called", "named", "call",
            "then", "show", "the", "and", "with", "from", "that", "this", "react",
            "component", "css", "road", "roadmap", "plan", "zip", "sandbox", "give", "me",
        }
        important_words = [w for w in words if len(w) > 2 and w.lower() not in ignored]
        if important_words:
            return self._normalize_identifier(important_words[0])
    ...
    def _extract_file_path(self, task: str) -> Optional[str]:
        ...
        component_file_match = re.search(r'([A-Za-z0-9_\-]+\.(?:jsx|tsx|js|ts))', task, re.IGNORECASE)
        if component_file_match:
            return f"frontend/src/components/{component_file_match.group(1)}"
        css_file_match = re.search(r'([A-Za-z0-9_\-]+\.css)', task, re.IGNORECASE)
        if css_file_match:
            return f"frontend/src/components/{css_file_match.group(1)}"
        explicit_filename_patterns = [
            r'(?:called|named|call it)\s+["\']?([A-Za-z0-9_\-]+\.(?:js|jsx|ts|tsx|py|css|html|json|txt))["\']?',
            r'["\']([A-Za-z0-9_\-]+\.(?:js|jsx|ts|tsx|py|css|html|json|txt))["\']',
        ]
        for pattern in explicit_filename_patterns:
            match = re.search(pattern, task, re.IGNORECASE)
            if match:
                filename = match.group(1)
                if filename.lower().endswith((".jsx", ".tsx", ".js", ".ts", ".css")):
                    return f"frontend/src/components/{filename}"
                return filename
        # First, try to extract directory + filename patterns like:
        # "create a file called HelloWorld.js inside ./frontend/src/components"
        # "create file X inside folder Y"
        dir_file_patterns = [
            # "called filename inside directory/path" or "called filename in folder/path"
            r'called\s+([\w.-]+\.(?:js|jsx|ts|tsx|py|css|html|json|txt))\s+(?:inside|in)\s+(?:the\s+)?(?:folder|directory)?\s*\.?/?([\w./_-]+)',
            # "filename inside/in folder/path"
            r'([\w.-]+\.(?:js|jsx|ts|tsx|py|css|html|json|txt))\s+(?:inside|in)\s+(?:the\s+)?(?:folder|directory)?\s*\.?/?([\w./_-]+)',
            # "inside/in folder/path create/save/write filename.ext"
            r'(?:inside|in)\s+(?:the\s+)?(?:folder|directory)?\s*\.?/?([\w./_-]+).+?(?:create|save|write|called)\s+.*?([\w.-]+\.(?:js|jsx|ts|tsx|py|css|html|json|txt))',
            # "put that file inside the folder ./path" - extract directory only
            r'(?:put|place|save|store)\s+(?:that\s+)?file\s+(?:inside|in)\s+(?:the\s+)?(?:folder|directory)?\s*\.?/?([\w./_-]+)',
            # "inside the folder ./path" - extract directory only (for when filename is auto-generated)
            r'inside\s+(?:the\s+)?(?:folder|directory)\s+\.?/?([\w./_-]+)',
        ]
        
        for pattern in dir_file_patterns:
            match = re.search(pattern, task, re.IGNORECASE)
            if match:
                groups = match.groups()
                if len(groups) == 2:
                    # Determine which group is filename and which is directory
                    if '.' in groups[0] and any(groups[0].endswith(ext) for ext in ['.js', '.jsx', '.ts', '.tsx', '.py', '.css', '.html', '.json', '.txt']):
                        filename, directory = groups
                    else:
                        directory, filename = groups
                    
                    # Clean up directory path
                    directory = directory.strip().rstrip('/').rstrip('\\')
                    # Remove trailing words like "folder" or "directory" if captured
                    directory = re.sub(r'\s*(?:folder|directory)$', '', directory).strip()
                    ...
                    # Directory only - return just the directory (filename will be auto-generated)
                    directory = groups[0].strip().rstrip('/').rstrip('\\')
                    directory = re.sub(r'\s*(?:folder|directory)$', '', directory).strip()
                    if directory.startswith('./'):
                        directory = directory[2:]
                    # Return directory with trailing slash to signal it's a directory-only result
                    return directory + "/"

        # Fallback to original patterns for simple path extraction
        patterns = [
    ...
    async def _determine_files(self, task: str, context: dict) -> list[dict]:
        ...
        # If file_path is a directory only (ends with /), combine with auto-generated filename
        if file_path and file_path.endswith("/"):
            directory = file_path.rstrip("/")
            generated_path = self._determine_file_path(task, language, framework, intent, component_name)
            filename = Path(generated_path).name
            file_path = f"{directory}/{filename}"
        # Determine appropriate file path if not specified
        elif not file_path:
    ...
    async def _build_api_registration_update(self, file_path: str) -> Optional[dict]:
        ...
        if import_line not in updated:
            # Prefer inserting after the app config import if present, otherwise insert after last import block
            anchor = "from app.core.config import settings\n"
            if anchor in updated:
                updated = updated.replace(anchor, f"{anchor}{import_line}\n", 1)
            else:
                # Find last top-level import and insert after it
        ...
        if include_line not in updated:
            # Insert after the last existing app.include_router(...) call if present
            last_include_pos = updated.rfind("app.include_router(")
            if last_include_pos != -1:
                # Find end of the line where the last include occurs
                line_end = updated.find("\n", last_include_pos)
                if line_end != -1:
                    updated = updated[:line_end+1] + include_line + "\n" + updated[line_end+1:]
                else:
                    updated = updated + "\n" + include_line + "\n"
            else:
                # Fallback: insert before optional routers loop if present, otherwise append at end
                loop_anchor = "for module_path, attr_name, label in _OPTIONAL_ROUTERS:"
                if loop_anchor in updated:
                    updated = updated.replace(loop_anchor, include_line + "\n\n" + loop_anchor, 1)
                else:
...
'''
    ...
    async def _read_file(self, file_path: str) -> tuple[bool, str]:
        """Read a file's contents (supports workspace staging)."""
        try:
            base = Path(self.project_root)
            if getattr(self, "use_workspace", False) and getattr(self, "workspace", None) is not None:
                base = Path(getattr(self.workspace, "base"))
...
def get_intelligent_coding_agent(project_root: Optional[Path] = None) -> IntelligentCodingAgent:
    ...
    elif project_root is not None and _agent.project_root != project_root:
        _agent = IntelligentCodingAgent(project_root)
    return _agent
