...
class TerminalIntegration:
    ...
    async def run_command(self, command: str, timeout: int = 120, cwd: str | None = None) -> TerminalOutput:
        """Run a command and capture output."""
        start_time = datetime.now(UTC)
        working_dir = self._resolve_cwd(cwd)
        
        # Security checks
        policy_decision = SecuritySandboxPolicy(working_dir).validate_command(command)
        if not policy_decision.allowed:
            return TerminalOutput(
                command=command,
                stderr=policy_decision.reason or "Command blocked by sandbox policy",
                return_code=-1,
