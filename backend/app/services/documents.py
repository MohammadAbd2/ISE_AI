...
from app.services.memory_summary import get_memory_summary_service
...
class DocumentService:
    ...
    async def build_context(
        ...
        context: list[str] = []
        if self._should_load_developer_docs(user_message):
            context.extend(self._load_developer_doc_context())
            context.extend(self.codebase_map.build_prompt_context())
        if self._should_load_runtime_self_context(user_message):
            context.extend(await self._load_runtime_self_context())
        total_chars = 0
    ...
    def _load_developer_doc_context(self) -> list[str]:
        candidates = [
            self.project_root / "docs" / "ISE_DEVELOPER_HANDBOOK.md",
        ...
        for path in candidates:
            ...
            block = (
                f"Developer reference: {path.relative_to(self.project_root)}\n"
                f"Purpose: Canonical repository documentation for architecture, extension points, and self-development.\n"
                f"Excerpt:\n{excerpt}"
            )
            if total_chars + len(block) > max_chars:
                break
    ...
    async def _load_runtime_self_context(self) -> list[str]:
        ...
        for capability in capabilities[:12]:
            ...
            if metadata.get("route"):
                detail_parts.append(f"route={metadata['route']}")
            if metadata.get("dashboard_panel"):
                detail_parts.append(f"panel={metadata['dashboard_panel']}")
            capability_lines.append(
                f"- {capability['name']} [{capability['status']}]"
                + (f" ({', '.join(detail_parts)})" if detail_parts else "")
            )

        memory_summary_service = self.memory_summary_service or get_memory_summary_service()
        summary = await memory_summary_service.build_summary()
        memory_lines = [
            f"- response_style: {summary['user_memory'].get('response_style') or 'none'}",
            f"- learned_preferences: {summary['user_memory'].get('learned_preferences', 0)}",
            f"- technologies: {', '.join(summary['project_memory'].get('technologies', [])) or 'none'}",
            f"- backend_services: {summary['project_memory'].get('backend_services', 0)}",
            f"- frontend_components: {summary['project_memory'].get('frontend_components', 0)}",
            f"- cached_queries: {summary['research_memory'].get('cached_queries', 0)}",
            f"- recent_turn_count: {summary['execution_memory'].get('recent_turn_count', 0)}",
            f"- recent_agents: {', '.join(summary['execution_memory'].get('recent_agents', [])) or 'none'}",
        ]
        return [
    ...
    def _should_load_developer_docs(self, user_message: str) -> bool:
        lower = user_message.lower()
        phrases = [
            "architecture",
            "documentation",
    ...
    def _should_load_runtime_self_context(self, user_message: str) -> bool:
        lower = user_message.lower()
        phrases = [
            "how can you develop yourself",
            "how can you develop your self",
