...
_PROJECT_ROOT = str(Path(settings.backend_root).parent.parent)
...
class AgentToolbox:
    ...
    def __init__(self, chat_service: ChatService, profile_service: ProfileService) -> None:
        ...
        self.filesystem_plugin = FileSystemPlugin(root_path=_PROJECT_ROOT)
    ...
    def _should_list_files(self, lower: str) -> bool:
        phrases = [
            ...
            "project tree",
            "directory tree",
            # File manipulation queries (write operations)
            "delete file",
            "delete the file",
            "remove file",
            "remove the file",
    ...
    async def _list_files_tool(self, user_message: str) -> ToolResult:
        ...
        try:
            ...
            if any(phrase in lower for phrase in ["search in files", "find in files", "find all in files", "grep in files"]):
            ...
            # Check if asking to delete a file
            if any(phrase in lower for phrase in ["delete file", "delete the file", "remove file", "remove the file"]):
                file_path = self._extract_file_path_from_query(user_message)
                if file_path:
                    result = self.filesystem_plugin.delete_file(file_path=file_path)
                    if result["success"]:
                        return ToolResult(name="File System", content=f"🗑️ **File deleted:** `{result.get('path', file_path)}`\n*Freed {result.get('size_freed', 0)} bytes*")
                    else:
                        return ToolResult(name="File System", content=f"❌ Error: {result.get('error', 'Unknown error')}")
                return ToolResult(name="File System", content="❌ Could not determine which file to delete. Please specify the file path.")

            # Check if asking to rename a file
            if any(phrase in lower for phrase in ["rename file", "rename the file"]):
                old_path, new_path = self._extract_rename_paths(user_message)
                if old_path and new_path:
                    result = self.filesystem_plugin.rename_file(old_path=old_path, new_path=new_path)
                    if result["success"]:
                        return ToolResult(name="File System", content=f"✏️ **File renamed:** `{old_path}` → `{new_path}`")
                    else:
                        return ToolResult(name="File System", content=f"❌ Error: {result.get('error', 'Unknown error')}")
                return ToolResult(name="File System", content="❌ Could not determine source and/or destination paths. Please specify both.")

            # Check if asking to copy a file
            if any(phrase in lower for phrase in ["copy file", "copy the file"]):
                source, dest = self._extract_copy_paths(user_message)
                if source and dest:
                    result = self.filesystem_plugin.copy_file(source_path=source, dest_path=dest)
                    if result["success"]:
                        return ToolResult(name="File System", content=f"📋 **File copied:** `{source}` → `{dest}`")
                    else:
                        return ToolResult(name="File System", content=f"❌ Error: {result.get('error', 'Unknown error')}")
                return ToolResult(name="File System", content="❌ Could not determine source and/or destination paths. Please specify both.")

            # Check if asking to write/update content to a specific file
            if any(phrase in lower for phrase in ["write to file", "write content to", "save to file", "update file", "update the file", "edit file", "edit the file"]):
                file_path, content = self._extract_write_paths(user_message)
                if file_path and content:
                    from app.services.tool_executor import ToolExecutor
                    executor = ToolExecutor()
                    try:
                        result = executor.write_file(path=file_path, content=content)
                        return ToolResult(name="File System", content=f"✏️ **File updated:** `{result['path']}`\n*Wrote {result['bytes_written']} bytes*")
                    except Exception as e:
                        return ToolResult(name="File System", content=f"❌ Error: {str(e)}")
                return ToolResult(name="File System", content="❌ Could not determine file path and/or content. Please specify both.")
    ...
    def _extract_search_pattern(self, user_message: str) -> str:
        ...
        # Patterns like "search in files for X" or "find X in files"
        patterns = [
            r"(?:search in files|find in files|grep in files)\s+(?:for\s+)?['\"]?([^'\"]+?)['\"]?(?:\s+in\s+[\w./-]+|$)",
            r"(?:search|find|grep)\s+(?:for\s+)?['\"]?([^'\"]+?)['\"]?\s+in files",
        ]
    ...
    def _extract_rename_paths(self, user_message: str) -> tuple[str | None, str | None]:
        """Extract old and new paths for rename operation"""
        import re
        lower = user_message.lower()

        # Pattern: "rename file old_path to new_path" or "rename old_path to new_path"
        patterns = [
            r"rename\s+(?:file\s+)?['\"]?([\w./_-]+)['\"]?\s+to\s+['\"]?([\w./_-]+)['\"]?",
            r"rename\s+['\"]?([\w./_-]+)['\"]?\s+(?:to|as)\s+['\"]?([\w./_-]+)['\"]?",
        ]
    ...
    def _extract_copy_paths(self, user_message: str) -> tuple[str | None, str | None]:
        """Extract source and destination paths for copy operation"""
        import re
        lower = user_message.lower()

        # Pattern: "copy file source to dest" or "copy source to dest"
        patterns = [
            r"copy\s+(?:file\s+)?['\"]?([\w./_-]+)['\"]?\s+to\s+['\"]?([\w./_-]+)['\"]?",
        ]
    ...
    def _extract_write_paths(self, user_message: str) -> tuple[str | None, str | None]:
        """Extract file path and content for write operation"""
        import re

        # Pattern: "write 'content' to file_path" or "update file_path with 'content'"
        patterns = [
            r"(?:write|save)\s+['\"]([^'\"]+)['\"]\s+(?:to|into)\s+['\"]?([\w./_-]+)['\"]?",
            r"(?:update|edit)\s+(?:file\s+)?['\"]?([\w./_-]+)['\"]?\s+(?:with|to)\s+['\"]([^'\"]+)['\"]?",
            r"(?:write|save)\s+(?:to\s+)?(?:file\s+)?['\"]?([\w./_-]+)['\"]?\s+(?:with\s+content|containing)\s+['\"]([^'\"]+)['\"]?",
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                groups = match.groups()
                if len(groups) == 2:
                    # Determine which group is the path and which is content
                    if '.' in groups[0] and '/' in groups[0]:
                        # First is path, second is content
                        return groups[0].strip(), groups[1].strip()
                    else:
                        # First is content, second is path
