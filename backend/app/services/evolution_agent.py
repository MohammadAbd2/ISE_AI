...
from app.services.knowledge_update_capability import KnowledgeUpdateCapability
...
class EvolutionAgent:
    ...
    async def develop_capability(
        ...
        try:
            ...
            if capability_name == "image_generation":
                ...
                success, message = await self._develop_video_generation()
            elif capability_name == "knowledge_update":
                success, message = await self._develop_knowledge_update()
            elif capability_name == "computer_use":
                success, message = await self._develop_computer_use()
            elif capability_name == "hardware_model_upgrade":
                success, message = await self._develop_hardware_model_upgrade()
            else:
                return EvolutionDecision(
                    action="decline",
                    capability=capability_name,
    ...
    async def _develop_knowledge_update(self) -> tuple[bool, str]:
        """Run the KnowledgeUpdateCapability research and plan workflow."""
        try:
            capability = KnowledgeUpdateCapability()
            report = await capability.run()
            self._knowledge_update_capability = capability
            return True, report
        except Exception as exc:
            return False, f"Knowledge update research failed: {exc}"

    async def _develop_computer_use(self) -> tuple[bool, str]:
        """Placeholder for computer-use / GUI automation development."""
        return True, (
            "🖥️ **Computer Use Development Plan**\n\n"
            "I will integrate into the sandbox:\n"
            "1. **Xvfb** — virtual framebuffer\n"
            "2. **PyAutoGUI** — mouse/keyboard automation\n"
            "3. **Playwright** — browser automation\n\n"
            "Reply **confirm** to begin installation."
        )

    async def _develop_hardware_model_upgrade(self) -> tuple[bool, str]:
        """Show hardware profile and recommended model upgrades."""
        from app.services.model_manager import get_model_manager
        mm = get_model_manager()
        hw = mm.get_hardware_info()
        return True, (
            f"## 🖥️ Hardware Profile\n\n"
            f"| Property | Value |\n|---|---|\n"
            f"| RAM Total | {hw['total_ram_gb']} GB |\n"
            f"| RAM Available | {hw['available_ram_gb']} GB |\n"
            f"| Safe Model Limit | {hw['safe_limit_gb']} GB |\n"
            f"| GPU VRAM | {hw['gpu_vram_gb']} GB |\n"
            f"| Tier | **{hw['tier']}** |\n\n"
            f"**Recommended Chat Model**: `{hw['chat_model']}`\n"
            f"**Recommended Coding Model**: `{hw['coding_model']}`\n\n"
            f"```bash\nollama pull {hw['coding_model']}\n```"
        )
    ...
    async def _develop_video_generation(self) -> tuple[bool, str]:
        """Implement video generation capability."""
        try:
            ...
            return False, f"Error developing video generation: {str(e)}"
