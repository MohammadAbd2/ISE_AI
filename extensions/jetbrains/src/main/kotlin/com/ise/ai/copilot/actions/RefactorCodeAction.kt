...
class RefactorCodeAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (selectedText.isNullOrBlank()) {
            Messages.showWarningDialog(project, "Please select code to refactor", "No Selection")
            return
        }
        
        val language = editor.virtualFile?.fileType?.name ?: "unknown"
        val prompt = "Refactor this $language code to improve readability and performance:\n\n```\n$selectedText\n```"
        ...
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                Messages.showInfoMessage(project, response, "Refactored Code")
            } catch (ex: Exception) {
