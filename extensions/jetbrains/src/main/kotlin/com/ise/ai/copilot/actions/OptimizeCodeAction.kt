...
class OptimizeCodeAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (selectedText.isNullOrBlank()) {
            Messages.showWarningDialog(project, "Please select code to optimize", "No Selection")
            return
        }
        
        val language = editor.virtualFile?.fileType?.name ?: "unknown"
        val prompt = "Optimize this $language code for better performance and efficiency:\n\n```\n$selectedText\n```"
