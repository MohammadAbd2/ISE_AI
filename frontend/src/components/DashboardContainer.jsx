...
const DynamicWidget = ({ 
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error(`Widget fetch error (${type}):`, err);
    } finally {
      setLoading(false);
    }
  }, [type, config]);
...
const WidgetContent = ({ type, data, config }) => {
  switch (type) {
    ...
    case 'promptfoo':
      return <PromptfooContent data={data} />;
    default:
...
/**
 * Promptfoo Content - Automated prompt testing results
 */
const PromptfooContent = ({ data }) => (
  <div className="promptfoo-container">
    <div className="promptfoo-summary">
      <div className="promptfoo-stat">
        <div className="promptfoo-label">Success Rate</div>
        <div className="promptfoo-value">
          {data.stats ? Math.round((data.stats.successes / (data.stats.successes + data.stats.failures)) * 100) : 0}%
        </div>
      </div>
      <div className="promptfoo-stat">
        <div className="promptfoo-label">Pass/Fail</div>
        <div className="promptfoo-value">
          {data.stats?.successes || 0}/{data.stats?.failures || 0}
        </div>
      </div>
    </div>
    <div className="promptfoo-results">
      {data.results && data.results.slice(0, 5).map((result, idx) => (
        <div key={idx} className="promptfoo-result-item">
          <div className="promptfoo-result-header">
            <span className={`promptfoo-status-pill ${result.success ? 'promptfoo-status-pass' : 'promptfoo-status-fail'}`}>
              {result.success ? 'PASS' : 'FAIL'}
            </span>
            <span className="promptfoo-score">Score: {result.score}</span>
          </div>
          <div className="promptfoo-prompt-text">{result.prompt}</div>
          {result.vars && Object.keys(result.vars).length > 0 && (
            <div className="promptfoo-vars">
              {Object.entries(result.vars).map(([k, v]) => (
                <span key={k} className="artifact-chip">{k}: {v}</span>
              ))}
            </div>
          )}
        </div>
...
const DashboardContainer = () => {
  ...
  const availableWidgets = [
    ...
    { type: 'promptfoo', title: 'Promptfoo Evals' }
  ];
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        ...
        console.warn('Could not load dashboard config, using defaults');
        initializeDefaultWidgets();
      } finally {
        setIsLoading(false);
      }
