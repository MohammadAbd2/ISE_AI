...
function isBraveBrowser() {
  ...
  return /Brave/i.test(navigator.userAgent || "") || Boolean(navigator.brave);
}

function normalizeVoiceError(errorCode) {
  switch (errorCode) {
    case "network":
      return "Voice recognition is unavailable right now. On desktop browsers this can happen when the connection is offline, the browser speech service is blocked, or the local HTTPS certificate is not trusted yet.";
    case "not-allowed":
...
export function useVoiceInput(onTextInsert) {
  ...
  const checkSupport = useCallback(() => {
    ...
    const isSecureContext = Boolean(window.isSecureContext) || isLocalhost;

    // Speech recognition works best on secure contexts.
    return (isLocalhost || isHTTPS) && isSecureContext;
  }, []);
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === "network") {
        ...
        const braveHint = isBraveBrowser()
          ? " Brave often blocks the browser speech service on localhost; disable Shields for this site or use Chrome/Edge for voice input."
          : "";
        setError(
          "Voice recognition is unavailable in this browser session. This usually means the browser speech service is blocked, unavailable on your network, or the local certificate is not fully trusted. Use text input for now, or install/trust a localhost certificate with mkcert and reload." + braveHint
        );
  ...
  useEffect(() => {
    ...
    if (!isSupported) {
      ...
      if (!SpeechRecognition) {
        ...
        setError("Voice input requires a trusted secure context. Open the HTTPS frontend and trust the local certificate, or use localhost.");
      }
      return undefined;
    }
