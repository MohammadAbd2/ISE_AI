export function classifyTaskIntent(text, mode = "auto") {
  ...
  const looksLikeVisualImplementation =
    visualTerms.some((term) => lower.includes(term)) &&
    ["component", "frontend", "file", "jsx", "tsx", "react", "render the result in the chat", "render it in the chat", "start coding", "develop this component", "build it"]
      .some((term) => lower.includes(term)) &&
    ["create", "build", "implement", "put", "make", "develop", "render"]
      .some((term) => lower.includes(term));
  ...
  if (looksLikeVisualImplementation) {
    return { kind: "coding", useAgent: true };
  }
